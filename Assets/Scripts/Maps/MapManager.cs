...
public class MapManager : Singleton<MapManager> {
    ...
    public override void Start(EventHelper helper) {
        ...
        helper.OnUpdateEvent += CheckClickInMap;
    }
    ...
    private void UpdateFind(RaycastHit2D hit) {
        if (Input.GetMouseButtonDown(0)) {
            if (hit.collider.tag == "MapCell") {
                HexCell dest = ObjectHelper.GetClass<HexCell>(hit.collider.transform.parent.gameObject);

                if (player.CurCell == dest) {
                    return;
                }

                Debug.Log("Set dest");
                //寻路
                PathHelper.GetFindPathResult(player.CurCell, dest);
                //寻路结束回到大地图模式
